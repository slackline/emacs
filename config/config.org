...
+ [X] ~ltex-ls-plus~ fails on loading (possibly other language servers too) for many files, big problem this.
...
*Solution*: Appears to have resolved itself, possibly after updates to ELPA installed org-mode.
...
+ [X] Work out the cause of the above. *Solution* appears to have resolved itself 🤷‍
+ [ ] Lists indent on new line (I've solved this before so have it in my existing config...somewhere), work out why its
