...
- [ ] Auto-tangle to ~~/.config/emacs/init.el~ on saving (change header and )
...
+ [ ] Black formatting isn't applied on saving ~.py~ files, will run manually though.
+ [ ] ~lsp-ltex-plus~ fails on loading (possibly other language servers too)
