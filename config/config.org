...
#+BEGIN_SRC emacs-lisp :tangle init.el
  (use-package git-link
    :ensure t
    :after (magit)
    ;; :bind (("C-c m g g" . git-link-dispatch)
    ;;         ("C-c m g g" . git-link-dispatch)
    ;;         ("C-c m g l" . git-link)
    ;;         ("C-c m g c" . git-link-commit)
...
I use Slack for work (it's slowly replacing email as the main interuptor) but I try not to look at work outside of work
so haven't adopted using Emacs to look at it. There is the [[https://github.com/isamert/emacs-slack][emacs-slack]] package if required though.
* Miscellaneous
<<sec:misc>>
