...
#+BEGIN_SRC emacs-lisp :tangle init.el
  (use-package emacs
    ...
    (add-to-list 'default-frame-alist '(vertical-scoll-bars . right))
    :config
...
agents. I used this before [[sec:ssh-agency][ssh-agency]] until it broke but haven't had 100% success with [[sec:ssh-agency][ssh-agency]] so keep it hanging
around.
...
(use-package keychain-environment
  :ensure t
  :bind (("C-c k" . 'keychain-refresh-environment))
...
[[https://github.com/emacsattic/glab][glab]] extends <<sec:ghub>> for [[https://gitlab.com][GitLab]].
...
#+BEGIN_SRC emacs-lisp :tangle init.el
  (use-package git-link
    :ensure t
    :after (magit)
    :bind (("C-c m g c" . git-link-commit)
...
(use-package org
  ...
  ;; (org-mode . (lambda () (add-hook 'after-save-hook #'ns/org-babel-export-on-save)))
...
are required which do the leg work/magic in the background. Many operating systems have these in their package manager
or they can be manually installed (e.g. [[https://github.com/ltex-plus/ltex-ls-plus][ltex-ls-plus]]). ~mason~ solves this by being an installed to the [[https://github.com/mason-org/mason-registry][mason-registry]]
which packages and [[https://mason-registry.dev/registry/list][lists]] pretty much all the language servers going.

Language servers I've installed

+ [[https://github.com/pappasam/jedi-language-server][jedi-language-server]] for Python
+ [[https://github.com/wader/jq-lsp][jq-lsp]] for ~jq~
+ [[https://ltex-plus.github.io/ltex-plus/][ltex-ls-plus]]
...
+ [[https://www.pydocstyle.org/][pydocstyle]]
+ [[https://pypi.org/project/pylint/][pylint]]
...
(use-package mastodon
  ...
  (setq mastodon-instance-url "https://mastodon.social"
	mastodon-active-user "redactedslackline")
  )
#+END_SRC
...
Emacs to look at it. There is the [[https://github.com/isamert/emacs-slack][emacs-slack]] package if required though.
...
[[https://github.com/astoff/devdocs.el][devdocs.el]] is an Emacs viewer for [[https://devdocs.io/][DevDocs]].
