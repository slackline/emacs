...
+ [ ] Automatic line wrapping doesn't appear to work (at least in org-mode but may be related to org-mode not kicking in
  properly). Have solved similar to this before so should be straight-forward to check/solve.
...
+ [ ] Don't get the option to ~diff~ when ~recover-this-file~
...
+ [X] Pre-commit hook output for Mypy is not formatted correctly.
...
+ [ ] Despite setting ~(setq org-gtd-update-ack "4.0.0")~ this seems to fail being set both on a clean  install and on
