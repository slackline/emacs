...
+ [X] Transparency isn't automatically set on starting. *Solution* : See commit ~bfdc95f~
...
+ [X] Mode-line is not as expected, very cluttered ~mood-line~ not loaded. *Solution*: See commit ~d85af0d~
...
+ [X] Modus/EF themes fail with ~No applicable method: modus-themes-get-themes~ on ~kimura~. Removing byte-compiled
  ...
  byte-compiled) but make sure it is being used. *Solution* : See commit ~604e7f5~

** ~magit~

+ [ ] Hook to add worktrees to the default view doesn't appear to be set and needed manually re-running.
...
#+BEGIN_SRC emacs-lisp :tangle init.el
  (use-package magit
    ...
    ;; perhaps?
    (setq remove-hook 'git-commit-mode #'turn-on-auto-fill)
    (setq remove-hook 'forge-post-mode #'turn-on-auto-fill)
    (setq add-hook 'after-save-hook 'magit-after-save-refresh-status)
    (setq add-hook 'magit-status-sections-hook #'magit-insert-worktrees t)
    ;; :hook
...
[[https://codeberg.org/martianh/fj.el][fj]] provides an interface to  [[https://forgejo.org/][ForgeJo]] and is used to power [[https://codeberg.org][Codeberg]]
