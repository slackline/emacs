...
Whilst I tend to explicitly state ~:ensure t~ for almost all packages here I also set ~use-package-always-ensure t~ and
~use-package-always-defer t~ so that if I forget it should still be installed and will defer by default (unless the
package explicitly has ~:demand t~ set).
