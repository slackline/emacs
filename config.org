...
properly). *Solution*: appears to now work.
...
** TODO ~magit~
*** TODO GFM mode for Forge

+ [ ] Having set the mode to be GFM for Forge posts it has clobbered all the key-bindings. Major mode is GFM from
  Markdown mode, how to get this as a minor mode/hook so major mode remains "Forge"
+ [ ] Disable line-wrapping in Forge buffers.
*** DONE Worktrees
