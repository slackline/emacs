...
- [X] Switch to using ~:custom~ over ~:config~ as described [[https://batsov.com/articles/2025/04/17/using-use-package-the-right-way/][here]].
...
- [X] Auto-tangle to ~~/.config/emacs/init.el~ on saving (change header and )
...
+ [X] Transparency isn't automatically set on starting. *Solution* : ~bfdc95f~
+ [X] Mouse wheel for scrolling isn't working - randomly started working on its own, keep an eye on this.
+ [X] No icons in tabs to indicate what filetype a buffer is. *Solution* :  ~M-x all-the-icons-install-fonts~.
+ [X] Mode-line is not as expected, very cluttered ~mood-line~ not loaded. *Solution*: ~d85af0d~.
+ [X] Automatic line wrapping doesn't appear to work (at least in org-mode but may be related to org-mode not kicking in
...
+ [X] Don't get the option to ~diff~ when ~recover-this-file~ *Solution*: appears to now work.
+ [X] Automatically create directories that do not exist (have this already shouldn't be hard to fix) *Solution*: ~234e8b1~
+ [ ] Files always result in ~setq-empty-p nil~, debug follows...
...
+ [X] Modus/EF themes fail with ~No applicable method: modus-themes-get-themes~ on ~kimura~. Removing byte-compiled
...
+ [X] Hook to add worktrees to the default view doesn't appear to be set and needed manually re-running. *Solution*:
  ~e04e6d9~ (after ~53363e0~)
+ [X] Pre-commit hook output for Mypy is not formatted correctly. *Solution*: It is now formatted (in colour) on
  completion of hooks being run.
+ [X] Virtual environment via ~direnv~ does not appear to be correctly loaded and being used in commits. *Solution*:
  ~9aec0d1~
