...
There are a lot of excellent resources to help guide Emacs users through configuring their Emacs as their own.
...
| Display local help for point (of cursor)     | ~C-h .~                                     |
...
+ ~:custom~ Avoids the need to prefix with ~setq~ (I think!).

The following table is constructed from the archived [[https://github.com/jwiegley/use-package][use-package README.md]] and the [[https://www.gnu.org/software/emacs/manual/html_mono/use-package.html][Emacs docs : use-package]] page since
it is now a core component of Emacs. use-package provides the [[https://www.gnu.org/software/emacs/manual/html_mono/use-package.html#Basic-Concepts][~use-package~ macro]] which simplifies configuration.
...
| ~:defer~       | Delay loading of package. Can be ~t~ but is rarely used, typically an integer /n/ is specified to load the package after that period of idle time. See [[https://www.jamescherti.com/emacs-startup-defer-use-package-performance/][this article]] for more info. | ~:defer t~ / ~:defer 30~                                                                                                                                                               |
...
| ~:map~         | Within a ~:bind~ you can provide the mapping and modify key
maps.                                                                                  | ~:bind (:map
projectile-command-map ("s-p p" . projectile-switch-project))~ / ~:bind (:map projectile-command-map ("s-p p" ("Switch
project" . projectile-switch-project)))~ (untested) |
...
#+BEGIN_SRC emacs-lisp :tangle no
  ...
  (add-to-list 'package-archives '("melpa" . "https://releases.melpa.org/packages/"))
  (package-initialize)
...
We can profile startup by ~(setq use-package-compute-statistics t)~ and then compiling a report with
~use-package-report~. For finer grained analysis the [[https://github.com/dholm/benchmark-init-el][benchmark-init-el]]
package can be used.
...
*** Autoupdates
<<sec:use-package-autoupdate>>

I've tried auto-updating packages in the past but have opted not to continue doing so as I often found errors snuck
in and it became challenging to work out what the problem was. Instead I update manually.

#+NAME: code-use-package-autoupdate
#+BEGIN_SRC emacs-lisp :tangle no
...
[[https://github.com/radian-software/straight.el][straight.el]] "/Next-generation, purely functional package manager for the Emacs hacker./" is potentially useful but I've
not yet explored it. It simplifies installing packages from Git Forges and keeping them up-to-date which is useful for
packages that aren't available on an existing [[sec:elpa][ELPA]].

** Byte Compiling
<<sec:byte-compiling>>

Byte-compilation takes your ASCII plain-text configuration and packages files saved in ~.el~ and turns them into
computer code (~.elc~) which can be loaded quicker. There are a couple of options available ~auto-compile~ and
~compile-angel~ being two that I have explored before. I have a hook to [[https://emacs.stackexchange.com/a/110][hide the compilation buffer]] on completion of
compiling as I got fed up of these being thrust into the foreground. I don't currently use either of these but keep them
hanging around in case I change my mind so I don't have to dig through my Git history to find the configuration.
...
+ [[https://emacsredux.com/blog/2026/04/07/stealing-from-the-best-emacs-configs/][Stealing from the Best Emacs Configs]] written by the author of [[sec:projectile][projectile]].
+ [[https://www.emacswiki.org/emacs/SetFonts#GloballyChangingFont][Changing the default font]]
+ [[https://emacs.stackexchange.com/a/78802/10100][How to disable automatic appearance of warnings buffer in emacs]] - set ~warning-minimum-level~ to ~:error~
...
I came across a neat way of showing a random item from Emacs news in a post by [[https://www.alcarney.me/blog/2026/emacs-tip-of-the-day/][Alex Carney]]. Haven't yet tried this and
not sure how useful it will be as I typically start Emacs in daemon mode and jump straight into doing something.
...
Emacs saves temporary versions of the last state of your file to ~*~~ files which is pretty handy if you aren't version
controlling your files already. Temporary buffers are saved to ~.#<filename>~ which is also useful. But there is more to
it than that and I found a useful [[https://brainbaking.com/post/2026/02/a-note-on-file-history-in-emacs/][Note on File History in Emacs]] enlightening with some handy suggestions on how to
improve things. I already use [[sec:vundo][vundo]] to give a crude visual history and navigate back through changes but this improved
my use of backups.
...
+ [[sec:org-agenda][Org Agenda]]
+ [[sec:quarto-mode][Quarto Mode]]
+ [[sec:sqlite][SQLite]]

*** Reversible ~winner-mode~
<<sec:reverisble-windows>>

#+NAME: code-reverisble-windows
#+BEGIN_SRC emacs-lisp :tangle init.el
...
Excellent tip from the [[https://emacsredux.com/blog/2022/06/12/auto-create-missing-directories/][Emacs Redux]] blog

#+NAME: ns/auto-create-missing-dirs
#+BEGIN_SRC emacs-lisp :tangle init.el
...
*** Convert word at point to Wikipedia link

A neat trick to convert word at point or text selection into a Wikipedia link. The [[http://ergoemacs.org/emacs/elisp_html_word_to_wikipedia_linkify.html][original]] inserted HTML formatted
links, I've tweaked it to output [[sec:org-mode][org-mode]] links.

#+NAME: ns/wikipedia-linkify
#+BEGIN_SRC emacs-lisp :tangle init.el
...
[[https://github.com/jamescherti/easysession.el/][easysession.el]] "/persist and Restore Emacs sessions (buffers, windows, tab-bar, scratch, Dired, narrowing, indirect buffers/clones, Magit buffers, scratch...); a robust desktop.el replacement/"

#+NAME: code-easysession
#+BEGIN_SRC emacs-lisp :tangle no
...
** Completion Preview
<<sec:completion-preview>>

[[https://eshelyaron.com/posts/2023-11-17-completion-preview-in-emacs.html][completion-preview]] provides a nice compact preview of completion options whilst typing rather than the large pop-up boxes.

#+NAME: code-completion-preview
#+BEGIN_SRC emacs-lisp :tangle init.el
...
[[https://github.com/oantolin/orderless][Orderless]] divides completion into space-separated components and matches candidates that match all of the components in
any order.
...
The [[https://github.com/jroimartin/outline-occur][outline-occur]] package was quickly [[https://lists.gnu.org/archive/html/emacs-devel/2026-07/msg00156.html][absorbed]] into Emacs main.

#+NAME: code-outline-occur
#+BEGIN_SRC emacs-lisp :tangle no
(use-package outline-occur
  :custom
...
A [[https://gist.github.com/mmarshall540/a12f95ab25b1941244c759b1da24296d][Gist on how to avoid ~+prefix~]] showing when there are nested key chords has been useful.
...
+ [[https://karthinks.com/software/persistent-prefix-keymaps-in-emacs/][Persistent prefix keymaps in Emacs]]
+ [[https://protesilaos.com/codelog/2024-01-29-emacs-prefix-map/][Emacs: define prefix/leader key (nested key maps) | Protesilaos]]
+ [[https://www.gnu.org/software/emacs/manual/html_node/elisp/Prefix-Keys.html][Prefix Keys (GNU Emacs Lisp Reference Manual)]]
...
[[https://github.com/nhojb/consult-magit][consult-magit]]

#+NAME: code-consult-magit
#+BEGIN_SRC emacs-lisp :tangle init.el
...
(use-package consult-gh
  ...
  (consult-gh-auth-post-switch . (lambda (&rest args) (setq consult-gh-default-orgs-list (consult-gh--get-current-orgs t)))))
...
** magit-difftastic
<<sec:magit-difftastic>>

[[https://github.com/rschmukler/magit-difftastic][magit-difftastic]] builds on and complements [[sec:difftastic][difftastic.el]] by /rendering diffs with [[https://github.com/Wilfred/difftastic][difftastic]] inside Magit's own buffers
(~magit-status, ~magit-diff-mode@, ~magit-revision-mode~), as collapsible, navigable Magit sections with full staging
support/. Currently this has been manually cloned as its not yet available on MELPA and I didn't want to go down the
[[https://github.com/radian-software/straight.el][straight.el]] rabbit hole right now.

#+NAME: code-magit-difftastic
#+BEGIN_SRC emacs-lisp :tangle init.el
...
[[https://github.com/emacsorphanage/git-gutter][git-gutter]] highlights rows that have changed in the side-border of a buffer.
...
** conflict-buttons
<<sec:conflict-buttons>>

#+NAME: code-conflict-buttons
#+BEGIN_SRC emacs-lisp :tangle init.el
...
[[https://codeberg.org/martianh/fj.el][fj]] provides an interface to [[https://forgejo.org/][ForgeJo]] which is used to power [[https://codeberg.org][Codeberg]].
...
[[https://codeberg.org/thanosapollo/emacs-forgejo][forgejo]] allows interaction with ForgeJo instances (including Codeberg). It requires tokens to be saved in
~~/authinfo.gpg~ and also depends on [[https://codeberg.org/thanosapollo/emacs-keymap-popup][~emacs-keymap-popup~]] which is from the same author (why they didn't reuse
functionality from the built-in [[sec:which-key][which-key]] or [[sec:transient][transient]] is anyone's guess).
...
(use-package forgejo
  ...
  :after (after_name)
  :custom
  (forgejo-hosts '(("https://codeberg.org") ("https://forgejo.nshephard.dev")))
  (forgejo-watch-rules '(("*" . "author:slackline")
                          ("*" . "author:neil")))
...
[[https://codeberg.org/thanosapollo/emacs-forgejo][forgejo-vc]] yet another package for interacting with ForgeJo instances. Not tried this yet but unlike [[sec:forgejo][forgejo]] it uses
[[sec:transient][transient]]. The author has written a useful [[https://thanosapollo.org/projects/emacs-forgejo/][guide]] to usage.

#+NAME: code-forgejo-vc
#+BEGIN_SRC emacs-lisp :tangle no
...
[[https://codeberg.org/halvin/agitjo][agitjo]] "/extends Magit with a new menu for [[https://forgejo.org/docs/latest/user/agit-support/][AGit-Flow]] operations/"
...
[[https://github.com/0WD0/majutsu][majutsu]] is "/Magit for jujutsu/". Not yet on MELPA so not tangled.
...
** magit-browse-commit
<<sec:magit-browse-commit>>

[[https://github.com/bbw9n/magit-browse-commit][magit-browse-commit]] allows opening of pull/merge request from a ~magit-blame~ buffer.

#+NAME: code-magit-browse-commit
#+BEGIN_SRC emacs-lisp :tangle init.el
...
** magit-pre-commit
<<sec:magit-pre-commit>>


[[https://github.com/DamianB-BitFlipper/magit-pre-commit.el][magit-pre-commit.el]] provides "/pre-commit integration for Magit/". From a Magit buffer use ~@~ to bring up a transient
menu and follow your nose from there.

#+NAME: code-magit-pre-commit
#+BEGIN_SRC emacs-lisp :tangle init.el
...
+ [[https://anonimno.codeberg.page/tracking worktime.html][org-clock-persist history]]
...
I've found I'm terrible at logging in and out of tasks, turns out I'm not the only one! This is a useful [[https://ray-on-emacs.blogspot.com/2026/07/modeling-browser-history-in-emacs.html][blogpost]] that
has some tips on how to keep track of when you worked. Ideally I wougld like a way for keeping track of which project I'm
working in (perhaps via [[sec:projectile][Projectile]]) and have a single file which logs time against the overall project as I'm not
bothered at the moment about granular time keeping for specific tasks (as then the task of logging time becomes a job in
and of itself).
...
Related is the work on [[https://github.com/kickingvegas/fj][fj]] from Charles Choi to make it easy to copy GitHub issues to Org format. It's still in early
development but something I will be keeping an ey on as it's more comprehensive than
*** Org Modern
<<sec:org-modern>>
...
I'm yet to switch to using Org-mode and its [[https://orgmode.org/manual/Publishing.html][publishing]] functions. I did notice there is a useful feature to publish RSS
feeds though but it is limited to using a monolithic org-file. Enter [[https://git.sr.ht/~taingram/org-publish-rss/][org-publish-rss]] ([[https://thibaut.dev/blog/org-publish-rss.html][via]]) which works on the one-file
per post principle which I much prefer.

#+NAME: code-org-publish-tags
#+BEGIN_SRC emacs-lisp :tangle init.el
...
[[https://github.com/abo-abo/org-download][org-download]] makes it easy to move images from A to B which includes inserting them into documents.

#+NAME: code-org-download
#+BEGIN_SRC emacs-lisp :tangle init.el
...
*** Org Wild Notifier
<<sec:org-wild-notifier>>

[[https://github.com/emacsorphanage/org-wild-notifier][org-wild-notifier]] fires off desktop notifications for [[sec:org-agenda][org-agenda]] links.

#+NAME: code-org-wild-notifier
#+BEGIN_SRC emacs-lisp :tangle init.el
...
[[https://github.com/GerardoCendejas/org-roam-stats][Org-roam-stats]] analyses your Org-roam files and presents a dashboard summary.

#+NAME: code-org-roam-stats
#+BEGIN_SRC emacs-lisp :tangle no
(use-package org-roam-stats
  :ensure t
  :bind (("C-c n S" . org-roam-stats-start))
  :after org-roam)
#+END_SRC
...
***** TODO Make these function take an argument so it can be reused.
...
**** Retrieve URL at point
<<sec:retrieve-url-at-point>>

Typically though I want to get a URL and [[https://emacs.stackexchange.com/questions/3981/how-to-copy-links-out-of-org-mode][this thread]] had a couple of alternatives, and I found [[https://emacs.stackexchange.com/a/50870/10100][this one]] to be closest to
what I was after which I found from a post by [[https://emacs.stackexchange.com/a/63051][NickD on Emacs StackExchange]],.

#+NAME: code-retrieve-url-at-point
#+BEGIN_SRC emacs-lisp :tangle init.el
...
[[https://github.com/spiperac/org-grimoire][org-grimoire]] is "/a static site generator for Emacs and Org mode. Configured and run entirely through your Emacs init
file./".

I wanted to move my blog from [[https://quarto.org][Quarto]] to a more Emacs centric system which had to be literate (as I write various bits of
code) but also extend it to have other sections such as a copy of this literate configuration. ~org-grimoire~ looks like
a relatively new kid on the blog but reading the authors [[https://strah.net/post/org-grimoire-static-site-generator-for-gnu-emacs.html][post about it]] it seems simple but comprehensive.

I will have to work out how to build the site in [[https://codeberg.org][Codeberg]] pages automatically and how to translate my old posts from
Quarto to Org-mode ([[https://pandoc.org][Pandoc]]) but that will give me something to focus on!

In the end I´ve gone with [[https://github.com/idlip/hakimacs][Hakimacs]].

#+NAME: code-org-grimoire
#+BEGIN_SRC emacs-lisp :tangle no
...
+ [[https://tilde.town/~cryptk/blog/yaob.html#installation][YAOB – Yet Another Org Blog · (interzone)]]
...
[[https://emacs.stackexchange.com/questions/80734/org-capture-to-bibtex-file][org-capture to bibtex file]] - useful StackExchange answer on capturing BibTex references to files, I only use a very
basic version of this as I use the [[https://github.com/Langenscheiss/bibitnow][BibItNow]] browser plugin to generate citations in the desired format which are ready
to paste in.
...
There are lots of programming languages, but Emacs has a mode for almost all of them.
...
found it to work consistently across buffers and [[sec:polymode][polymode]] seems to be a particular problem.
...
[[https://codeberg.org/teoten/esr][esr]] is a new package that is being developed with the goal of simplifying and focusing solely on Emacs' interaction with
...
Instead I found a custom Quarto configuration from [[https://github.com/aceross/emacs/commits/master/site-elisp/quarto-mode.el][aceross/emacs]]'s configuration and used that. It is included below
verbatim as of 2025-02-15. Unfortunately as of <2026-03-07 Sat> its not working so not currently tangled.
...
[[https://codeberg.org/R-for-emacs/r-ts-mode][r-ts-mode]] is a "/major mode for editing R code using treesitter/".
...
[[https://codeberg.org/zshaftel/lisp-ts-mode][lisp-ts-mode]]

#+NAME: code-lisp-ts-mode
#+BEGIN_SRC emacs-lisp :tangle no
(use-package lisp-ts-mode
  ...
  :hook (lisp-ts-mode-hook . lisp-ts-format-support-mode))
...
[[https://github.com/dnouri/md-ts-mode][md-ts-mode]] an alternative [[sec:treesitter][Tree Sitter]] mode.

#+NAME: code-md-ts-mode
#+BEGIN_SRC emacs-lisp :tangle no
...
I found that many of the above hooks were not actually being activated on opening Python files (which triggers
~python-mode~ as the major mode and things like ~blacken-mode~, ~ruff-format-on-save-mode~ etc. should be minor modes).

#+NAME: code-python-hooks
#+BEGIN_SRC emacs-lisp :tangle init.el
...
An alternative which looks interesting as it avoids the need to have multiple language servers running is [[https://github.com/jacktasia/dumb-jump][dumb-jump]]
(encountered via [[https://codelearn.me/2026/01/03/dumb-jump-is-so-good.html][Emacs: Dumb Jump Is So Good]]) which I've not tried but like the sound of.
...
I'd like to be able to log in and out of a project specific [[sec:org-mode][Org]] file when switching between projects. I found a possible
solution on [[https://emacs.stackexchange.com/a/42204][Emacs Stackexchange]] . There is also the related ~org-clock-[in|out]~ [[https://emacs.stackexchange.com/questions/41045/save-an-org-mode-buffer-when-i-clock-in-out-of-a-task][hooks]].

The thread also links to [[https://github.com/novoid/Memacs][Memacs]] which monitors what you are doing and looks like overkill for my use case.

#+NAME: code-projectile-org-log
#+BEGIN_SRC emacs-lisp :tangle init.el
;; Initially set to 0
...
As of [[https://www.rahuljuliato.com/posts/emacs-31-around-the-corner][Emacs 31]] treesitter grammars can be installed automatically and modes enabled. One exception is the experimental
~markdown-ts-mode~ mentioned in the article. One to keep an eye on.
...
FlySpell]] is a method of spell-checking a buffer on the fly. A useful article on configuring [[https://www.jamescherti.com/emacs-spell-checker-flyspell-ispell-aspell/][Flyspell, ispell and aspell]]
is something I haven't yet worked through and implemented.
...
+ [[https://www.jamescherti.com/emacs-the-definitive-guide-to-code-folding/][Emacs: The Definitive Guide to Code Folding | James Cherti]]
...
**** outline-minor-mode
<<sec:outline-minor-mode>>

#+NAME: code-outline-minor-mode
#+BEGIN_SRC emacs-lisp :tangle init.el
...
[[https://www.gnu.org/software/emacs/manual/html_node/emacs/Hideshow.html][Hideshow]] is "/a buffer-local minor mode that allows you to selectively display portions of a program, which are referred
to as blocks./".

#+NAME: code-hs-minor-mode
#+BEGIN_SRC emacs-lisp :tangle init.el
...
[[https://github.com/jamescherti/outline-indent.el][outline-indent]] provides code folding based on indentation.

#+NAME: code-outline-indent
#+BEGIN_SRC emacs-lisp :tangle init.el
...
[[https://github.com/laluxx/porg/][porg]] bring org-mode functionality to ~prog-mode~. This includes nice code-folding. Not currently tangled though.
...
I started using this but found in this new configuration it didn't always work as expected. I then stumbled across [[sec:envrc][envrc]]
which I'm trialling for now. Ergo this isn't currently tangled.
...
[[https://github.com/purcell/envrc][envrc]] is an alternative to [[sec:direnv][direnv]], as [[https://github.com/purcell/envrc#how-does-this-differ-from-direnvel][noted]] rather than switching the global environment sets the environment on a
per-buffer basis. The one thing I might have to do is move this section to later in the configuration file as described
as global minor modes need to be enabled.
...
[[https://code.librehq.com/qhong/crdt.el][crdt.el]] allows real time collaborative writing on shared documents directly from Emacs.
...
I don't use the shell/terminal in Emacs much as I quite like having clear separate usage and so use [[https://sw.kovidgoyal.net/kitty/][kitty]] and [[https://tmux.app/][tmux]].
...
[[https://github.com/dakra/ghostel][ghostel]] is "/Emacs terminal emulator powered by [[https://ghostty.org/][libghostty-vt]]/". I'm in two minds about this, it sounds good from others
reviews/comments, but the author uses "large language models" heavily in the development.
...
+ [[https://brainbaking.com/post/2026/01/customizing-the-emacs-email-experience-with-mu4e/][Customizing The Emacs Email Experience With Mu4e]]
...
[[https://github.com/keenban/signel][signel]] is an Emacs client for Signal. It depends on [[https://github.com/AsamK/signal-cli][signal-cli]] an unofficial commandline, JSON-RPC and dbus interface
...
[[https://github.com/larsmagne/meme][meme]] is a small library for adding text to images (aka making memes). The package isn't get on MELPA and I don't have
any need to use it at the moment but I include it here as a reference/bookmark should that change.
...
[[https://github.com/dajva/rg.el][rg]] is a search tool based on [[https://github.com/BurntSushi/ripgrep][ripgrep]] and is required to make full use of [[sec:projectile][Projectile's]] searching of projects.
...
[[https://github.com/josegpt/display-wttr][display-wttr]] grabs data from [[https://wttr.in][wittr.in]] and will display it on the modeline.

#+NAME: code-display-wttr
#+BEGIN_SRC emacs-lisp :tangle init.el
...
[[https://github.com/cjennings/emacs-wttrin][wttrin]] is another interface to grabbing weather data from [[https://wttr.in][wittr.in]] and displaying it but in a dedicated buffer.
...
#+NAME: code-devdocs-to-install
#+BEGIN_SRC emacs-lisp :tangle no
(setq devdocs-install-alist
      '(("bash")
        ("elisp")
        ("git")
        ("go")
...
#+NAME: code-devdocs-install
#+BEGIN_SRC emacs-lisp :tangle no
(deffun ns/install-dev-docs (list)
 "Install devdocs.")
#+END_SRC
...
I like being able to read RSS feeds in Emacs but also want to read them on the-go (using ). I therefore maintain my own
instance of [[https://freshrss.org/][FreshRSS]] and point the excellent [[https://github.com/emacs-elfeed/elfeed][Elfeed]] to the global RSS feed of my FreshRSS instance. All very meta!
...
[[https://github.com/remyhonig/elfeed-org][elfeed-org]] allows you to configure [[sec:elfeed][Elfeed]] using an Org file. It allows hierarchical tags from the org structure to be
inherited, simplifying tags. I used to use this when I used Elfeed more, but since switching to FreshRSS I only have a
single RSS feed to read so no longer maintain my ~elfeed.org~ referenced below.

#+NAME: code-elfeed-org
#+BEGIN_SRC emacs-lisp :tangle no
...
<<sec:elfeed-protocol-freshrss>>

#+NAME: code-elfeed-protocol-freshrss
#+BEGIN_SRC emacs-lisp :tangle init.el
...
Elements concerned with the User Interface which typically means how buffers, mini-buffers, modeline look and behave.
...
[[https://git.andros.dev/andros/comet-trail.el][comet-trail]]

#+NAME: code-comet-trail
#+BEGIN_SRC emacs-lisp :tangle init.el
...
I use the [[https://github.com/source-foundry/Hack][Hack]] font in Emacs, it needs installing at the system level so it's available for Emacs. I also wanted a
slightly larger font on my laptop (my eyes are getting old 👴)
...
access the same list. It doesn't currently work with [[sec:mood-line][mood-line]] so isn't tangled.
...
Not currently tangled, switched to [[sec:moody][moody]] instead.
...
[[https://github.com/tarsius/moody][moody]] adds a tab to the mode-line, not sure if it works with [[sec:mood-line][mood-line]] so not currently tangled.
...
[[https://github.com/emacsmirror/buffer-guardian][buffer-guardian]] helps ensure that buffers are always saved. This sounded good, but as I have hooks to     ~(before-save
. delete-trailing-whitespace)~ and ~(before-save . do-auto-fill)~ I found it mildly annoying as the saves would occur
mid-typing. This wasn't a consequence of having set ~(buffer-guardian-save-all-buffers-idle 30)~ either.


#+NAME: code-buffer-guardian
#+BEGIN_SRC emacs-lisp :tangle no
...
ibuffer is built-in. A useful example of configuration is [[https://olddeuteronomy.github.io/post/emacs-ibuffer-config/][here]].
...
[[https://github.com/purcell/ibuffer-vc][ibuffer-vc]] is a neat extension which groups buffers in [[sec:ibuffer][ibuffer]] by version control groups. A related package (from the
same author!) is [[https://github.com/purcell/ibuffer-projectile][ibuffer-projectile]] which groups files by [[sec:projectile][projectile projects]].

#+NAME: code-ibuffer-vc
#+BEGIN_SRC emacs-lisp :tangle init.el
...
#+BEGIN_SRC emacs-lisp :tangle no
  (use-package indent-bars
    :ensure t
    :custom
    (setq indent-bars-color-by-depth t)
...
code block that is indented and ending it) are highlighted as you scroll. Have gone with [[sec:indent-bars][indent-bars]] instead
...
*** Project Tab Groups
<sec:project-tab-groups>>

[[https://github.com/fritzgrabo/project-tab-groups][project-tab-groups]] provides functions for grouping and switching between groups of tags that complements the built-in
~project.el~. Since I use [[sec:projectile][projectile]] I've not given this a whirl but am noting it for possible future use.
...
+ [[https://karthinks.com/software/emacs-window-management-almanac/][The Emacs Window Management Almanac | Karthinks]]
...
over-ridden. Useful for writing the ~early-init.el~ file.
...
If there is a particular code block I _don't_ want to tangle and include it is possible to disable it by setting ~:tangle
no~.
...
+ [[https://www.emacswiki.org/emacs/Carnival][EmacsWiki: Carnival]]
...
+ [[https://web.archive.org/web/20240525044519/https://themagitian.github.io/posts/emacsconfig/][Configuring Emacs - Magitian]] covers lots of things I use, very useful
...
+ [[https://alhassy.com/emacs.d/][A Life Configuring Emacs]]
+ [[https://www.rahuljuliato.com/posts/emacs-solo-two-years][Two Years of Emacs Solo: 35 Modules, Zero External Packages, and a Full Refactor | Rahul's Blog]]
...
+ [[https://tecosaur.github.io/emacs-config/config.pdf][Doom Emacs Configuration]]
+ [[https://sachachua.com/dotemacs/index.html][Sacha Chua's Emacs Configuration]] [[https://sachachua.com/dotemacs/index.pdf][PDF]]
+ [[https://protesilaos.com/codelog/2026-04-30-emacs-decent-default-sacha-chua/][Emacs: decent defaults I shared with Sacha Chua | Protesilaos]]
+ [[https://codeberg.org/jamesendreshowell/emacs-config][jamesendreshowell/emacs-config: My public Emacs configuration files - Codeberg.org]]
+ [[https://github.com/jamescherti/minimal-emacs.d][jamescherti/minimal-emacs.d: A Customizable Emacs init.el and early-init.el | Emacs Starter Kit]]
+ [[https://rossabaker.com/configs/emacs/khlav-kalash/][Ross A. Baker: Khlav Kalash Emacs]]
+ [[https://outputerror.com/emacs-config/][Output Error :: Emacs config]] very minimal
+ [[https://github.com/KallDrexx/emacs-zero-to-ide-journey][Emacs : Zero to IDE Journey]]
+ [[https://sachachua.com/blog/2026/04/what-s-in-the-emacs-newcomers-presets-theme/][What's in the Emacs newcomers-presets theme? :: Sacha Chua]]
...
+ [[https://rossabaker.com/blog/underappreciated-emacs-built-ins/][Ross A. Baker: Underappreciated Emacs built-ins]]
+ [[https://www.rahuljuliato.com/posts/emacs-31-around-the-corner][Emacs 31 Is Around the Corner: The Changes I'm Already Daily Driving | Rahul's Blog]]

** Performance

+ [[https://www.jamescherti.com/emacs-why-use-setq-instead-setopt/][Emacs startup: Why setq beats setopt, customize-set-variable, and use-package :custom? | James Cherti]]

** Packages

+ [[https://www.jamescherti.com/essential-emacs-packages/][Must-have Emacs Packages for Efficient Software Development and Text Editing | James Cherti]]
...
* Appendix : ToDo Trouble Shooting

Testing the new configuration I've found the following aren't quiet working properly.

** TODO Improvements

James Cherti seems to know what he's talking about and also shares his knowledge via a blog. The following are things I
should improve.

+ [X] [[https://www.jamescherti.com/emacs-startup-defer-use-package-performance/][Optimizing Emacs startup - Guide to deferred package loading with use-package | James Cherti]]
+ [ ] Update to follow the recommendations to work with [[https://www.jamescherti.com/emacs-spell-checker-flyspell-ispell-aspell/][ispell and aspell]].
+ [ ] Improve scrolling a lá [[https://www.jamescherti.com/emacs-scrolling-better-performance-usability/][Configuring Emacs Scrolling for Better Performance and Usability | James Cherti]]
+ [ ] [[https://www.jamescherti.com/emacs-fix-org-mode-copy-paste-yank-bleed/][Copy-paste without Emacs org-mode or markdown-mode formatting bleeding into other buffers]]
+ [ ] [[https://www.jamescherti.com/emacs-why-use-setq-instead-setopt/][Why use setq instead  of setopt]]
+ [ ] [[https://www.jamescherti.com/measuring-emacs-startup-time/][Measuring Emacs Startup Time More Accurately than the Built-in emacs-init-time | James Cherti]]

** General

*** TODO Improve use-package hooks

The [[https://www.gnu.org/software/emacs/manual/html_mono/use-package.html#Hooks][docs on hooks]] show that ...

#+begin_example elisp
(use-package: something
  :hook (prog-mode . something-mode))
#+end_example
...
*** TODO Auto-hide buffers
Hide ~*Messages*~ / ~*Compile Log*~ / ~*Error*~ buffers (have solved this before so have the
  solution...[[https://emacs.stackexchange.com/questions/62/hide-compilation-window][somewhere]]).

*** TODO ~delete-trailing-whitespace-mode~ / ~whitespace-mode~

This mode can be used to do what it says, but perhaps more cumbersome to enable against ~prog-mode~ / ~text-mode~
etc. than the current setup? What about ~whitespace-mode~ which shows whitespace in different manners and includes it as
a ~whitespace-action~ (see [[https://chaos.social/@citizen428/117314740975603221][thread]]).

*** TODO Mouse wheel
Scrolling with mouse wheel is intermittent - randomly started working on its own on some machines, keep an eye on this.
*** DONE ~compilation-finish-function~ hook
CLOSED: [2026-09-18 Fri 20:31]
Test the ~compilation-finish-function~ hook will kill compilation buffers. I think this works.
*** TODO which-key
Ensure ~(desc . command)~ is used for all custom key-bindings so that they are all shown by ~which-key~
*** DONE Auto-tangle
- [X] Auto-tangle to ~~/.config/emacs/init.el~ on saving, also outputs to HTML. *Solution*: missed this one!
*** DONE Transparency
+ Transparency isn't automatically set on starting. *Solution* : ~bfdc95f~
*** DONE Tab icons
No icons in tabs to indicate what filetype a buffer is. *Solution* :  ~M-x all-the-icons-install-fonts~.
*** DONE modeline
Mode-line is not as expected, very cluttered ~mood-line~ not loaded. *Solution*: ~d85af0d~.
*** DONE line wrapping
Automatic line wrapping doesn't appear to work (at least in org-mode but may be related to org-mode not kicking in
properly). *Solution*: appears to now work ~visual-line-mode~ 🤦.
*** DONE diff recovering files
Don't get the option to ~diff~ when ~recover-this-file~ *Solution*: appears to now work.
*** DONE automatically create dirs
Automatically create directories that do not exist (have this already shouldn't be hard to fix) *Solution*: ~234e8b1~
*** DONE Errors opening files

Opening files always result in ~setq-empty-p nil~, debug follows...

#+BEGIN_EXAMPLE
Debugger entered--Lisp error: (cl-no-applicable-method seq-empty-p nil)
  ...
  seq-empty-p(nil)
  seq-reduce(#f(compiled-function (acc elt) #<bytecode 0x57b5fc053859446>) nil nil)
  seq-difference(nil ("~/.config/emacs/" "~/org/" "~/org-roam/"))
  projectile-merge-known-projects()
  projectile-add-known-project("/home/neil/.config/emacs/")
  projectile-track-known-projects-find-file-hook()
  projectile-find-file-hook-function()
  run-hooks(find-file-hook)
  after-find-file(nil t)
  find-file-noselect-1(#<buffer init.el> "~/.config/emacs/init.el" nil nil "~/.config/emacs/init.el" (3026722 66307))
  find-file-noselect("~/.config/emacs/init.el" nil nil t)
  find-file("~/.config/emacs/init.el" t)
  funcall-interactively(find-file "~/.config/emacs/init.el" t)
  command-execute(find-file)
#+END_EXAMPLE
*** WONT-DO Buffer guardian

[[https://github.com/emacsmirror/buffer-guardian][buffer-guardian]]I think this might be causing some problems with my configuration editing, although I also see...

#+begin_example
⛔ Warning (flycheck): Syntax checker lsp reported too many errors (3848) and is disabled.
...
Modus/EF themes fail with ~No applicable method: modus-themes-get-themes~ on ~kimura~. Removing byte-compiled
~modus-themes*/*.elc~ and disabling loading of ~ef-themes~ solves this, but not an ideal solution. Consider submitting
issue but [[https://github.com/protesilaos/ef-themes/issues/63][this thread]] suggests that it has to be a new version of modus themes and not the one bundled in Emacs so
perhaps need to force downloading and installing both ~modus-themes~ (which appears to have been downloaded and
byte-compiled) but make sure it is being used. *Solution* : ~604e7f5~

** TODO ~magit~
*** DONE GFM mode for Forge
CLOSED: [2026-09-02 Wed 11:49]

+ [X] Having set the mode to be GFM for Forge posts it has clobbered all the key-bindings. Major mode is GFM from
  Markdown mode, how to get this as a minor mode/hook so major mode remains "Forge"
+ [X] Enable ~lsp-ltex-plus~ for ~forge-post-mode~. *Solution*: ~e4535ef~
+ [X] Disable line-wrapping in Forge buffers. *Solution*: ~e88e54f~

*** DONE Worktrees

Hook to add worktrees to the default view doesn't appear to be set and needed manually re-running. *Solution*:
~e04e6d9~ (after ~53363e0~)
*** DONE Pre-commit

Pre-commit hook output for Mypy is not formatted correctly. *Solution*: It is now formatted (in colour) on
 completion of hooks being run.
*** DONE direnv
Virtual environment via ~direnv~ does not appear to be correctly loaded and being used in commits. *Solution*:
~9aec0d1~

** TODO LSP mode

Mostly seems to be working but...

*** TODO R Air

A new linter and language server to be [[https://posit-dev.github.io/air/editor-emacs.html#lsp-mode][integrated]].

*** TODO Icons

+ [ ]  No icons in the bar across the top to show what the language is. Possibly down to [[https://github.com/emacs-lsp/lsp-mode/issues/2504][all-the-icons not being
  installed]]. Solution : ~M-x all-the-icons-install-fonts~.

*** TODO Python/Black

+ [ ] Black formatting isn't applied on saving ~.py~ files, will run manually though (down to a hook perhaps?).

*** TODO Blacklist root directory

+ [ ] LSP black-listed ~/home/neil/~ and so no nested directories initiated LSP sessions. May be related to ~projectile~
  which includes ~/home/neil~ and ~~/~ for some annoying reason.

*** DONE Java VM (under Gentoo)
+ [X] ~ltex-ls-plus~ fails on loading (possibly other language servers too) for many files, big problem
  this. *Solution*: ~dc5b75f~ (installing ~ltex-ls-plus~ via [[sec:mason][mason]] solved this)

+ [X] On Gentoo I also had to switch the Java VM to a newer version for users.

#+begin_example
Error: LinkageError occurred while loading main class org.bsplines.ltexls.LtexLanguageServerLauncher
	java.lang.UnsupportedClassVersionError: org/bsplines/ltexls/LtexLanguageServerLauncher has been compiled by a more recent version of the Java Runtime (class file version 65.0), this version of the Java Runtime only recognizes class file versions up to 55.0

Process ltex-ls-plus stderr finished
#+end_example

#+begin_example
❱ eselect java-vm --list
Available Java Virtual Machines:
  [1]   openjdk-bin-11  user-vm
...
Available Java Virtual Machines:
  [1]   openjdk-bin-11
  [2]   openjdk-bin-21  system-vm user-vm
  [3]   openjdk-bin-25
#+end_example

** DONE Org mode version mismatch
...
+ [X] Work out the cause of the above. *Solution* appears to have resolved itself after reinstalling and byte-compiling
  org-mode anew 🤷‍
+ [X] Lists indent on new line work out why its not loading correctly. *Solution* - not explicitly solved but it was
  because ~electric-indent-local-mode~ was not set to ~0~ (i.e. disabled). Hook was in place.

** DONE Org-GTD

+ [X] Despite setting ~(setq org-gtd-update-ack "4.0.0")~ this seems to fail being set both on a clean install and on
      subsequent restarts. Perhaps this is because it is only set via ~:custom~ so the option isn't set until the
      package is used? *Solution*: Move the setting to /before/ loading the package!

** DONE pytest

+ [X] Additional entries to the transient are not appearing.

** TODO Functions

+ [ ] Custom functions for markdown > org and org > markdown don't appear to be available.

** DONE Completion preview

+ [X] Add key-bindings.  *Solution* : ~22c042e~
